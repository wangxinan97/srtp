...
void Task0_PointFre(u32 Key_Value)//正弦波 (10M) 0-100 000 000
{
	...
	if(Key_Value != K_NO)
	{
		...
		Freq_convert(SinFre);
		delay_ms(20); //待波形稳定之后再采集
		
		ADCConvertedValueLocal[0] = ADCConvertedValue[0]; //获取幅值比的AD转换值
		ADCConvertedValueLocal[1] = ADCConvertedValue[1]; //获取相位差的AD转换值
		Precent[0] = (ADCConvertedValueLocal[0]*100/0x1000);	//算出百分比
    Precent[1] = (ADCConvertedValueLocal[1]*100/0x1000);	//算出百分比
		Voltage[0] = Precent[0]*33;						  // 3.3V的电平，计算等效电平
    Voltage[1] = Precent[1]*33;						  // 3.3V的电平，计算等效电平
		//串口打印
		printf("%4d,%4d", Voltage[0],Voltage[1]);
				
		if(Param_Mode == 1)//幅值比AD转换设置
		{
			P_Index %= 4;//参数位数
		  sprintf(fre_buf, "%4d", Voltage[0]);//字符转换
		}
		//显示
		sprintf(showstr, "%4d", Voltage[0]);//字符转换
		fre_buf_change(showstr);//fre_buf当中 ‘ '->'0'
		Copybuf2dis(showstr, display, P_Index, 0, 1);
		display[4]=0;//只显示3位
		...
		if(Param_Mode == 2)//相位差AD转换设置
		{
			P_Index %= 4;//参数位数
		  sprintf(fre_buf, "%4d", Voltage[1]);//字符转换
		}
		//显示
		sprintf(showstr, "%4d", Voltage[1]);//字符转换
		fre_buf_change(showstr);//fre_buf当中 ‘ '->'0'
		Copybuf2dis(showstr, display, P_Index, 0, 1);
		display[4]=0;//只显示3位
  ...
  u32 SinFre = SweepMinFre;
	...
	if(Key_Value != K_NO)
	{
		...
		if(Param_Mode == 2)//频率步进设置
		{
			...
			sprintf(fre_buf, "%7d", SweepStepFre);//数据重新写入
		}
		//显示
		sprintf(showstr, "%7d", SweepStepFre);//重新申请缓存显示
		fre_buf_change(showstr);//fre_buf当中 ‘ '->'0'
      ...
      Voltage[0] = Precent[0]*33;						  // 3.3V的电平，计算等效电平
		  Precent[1] = (ADCConvertedValueLocal[1]*100/0x1000);	//算出百分比
      Voltage[1] = Precent[1]*33;						  // 3.3V的电平，计算等效电平
			printf("%4d,%4d\n", Voltage[0],Voltage[1]);//串口打印
			delay_ms(80);
			SinFre = SinFre + SweepStepFre;
		}
      
		if(Param_Mode == 3)//AD转换设置
		{
			P_Index %= 4;//参数位数
			sprintf(fre_buf, "%4d", Voltage[0]);//字符转换
		}
